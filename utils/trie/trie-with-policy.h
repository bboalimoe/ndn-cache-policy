...
{
public:
	/// PayloadTraits ::   ndnSIM::smart_pointer_payload_traits< EntryImpl< ContentStoreImpl< Policy > >, Entry >
  typedef trie< FullKey,
                PayloadTraits,
                typename PolicyTraits::policy_hook_type > parent_trie;

  typedef typename parent_trie::iterator iterator;     /////namely a pointer to the trie
  typedef typename parent_trie::const_iterator const_iterator;

  ///////   the ::type is the struct implent the insert and update!!
  ...
  typedef typename PolicyTraits::template policy<
   ...
   /////intrusive list 中可以放 trie * 类型的变量,所以就是个container.
    /////class type 继承  public policy_container
  ///////  policy_container --> 是继承    lfu-save-tag-policy -->的line 128 的 multiset (Container ,

  inline
  trie_with_policy (size_t bucketSize = 1, size_t bucketIncrement = 1)
    : trie_ (name::Component (), bucketSize, bucketIncrement)     ///这里初始化后,就没有再操作,所以这是root node
    , policy_ (*this)
  {
  }
  ...
  insert (const FullKey &key, typename PayloadTraits::insert_type payload)
  {
    ...
    std::pair<iterator, bool> item =
      trie_.insert (key, payload);  /// payload is the entryimpl

    if (item.second) // real insert
      {
        bool ok = policy_.insert (s_iterator_to (item.first));      //这里是插入的真正地方
        if (!ok)
          {
  ...
  erase (const FullKey &key)
  {
    ...
    bool reachLast;
    //////  trie_find()   ---->   return boost::make_tuple (foundNode, reachLast, trieNode);
