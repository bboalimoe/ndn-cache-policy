...
#include "ns3/ndnSIM/model/cs/content-store-with-stats-inherit.h"
...
StackHelper::Install (Ptr<Node> node) const
{
  ...
  /*
   ..inherit the object....
   ..把每个继承自object的对象实例绑定在一个aggregates上,然后从ndn->getObject<fib>等从aggregates中取出
      对象实例.
      struct Aggregates {
      uint32_t n;
      Object *buffer[1];    //只有一个数组......
  };
  */
 ...
 // std::cout << "node " << node->GetId() << "get all the stacks" << std::endl;
