...
Producer::Producer ():m_servHitsForInstance(5)
{
...
Producer::StartApplication ()
{
  ...
  NS_LOG_DEBUG(  "producer app START " );
  ...
  Simulator::Schedule(Seconds(5), &Producer::PrintServerHits,this) ;
}
...
Producer::StopApplication ()
{
 ...
 Time Producer::TrackTime = Seconds(1000000);

void
Producer::PrintServerHits()
{

	if(TrackTime == Simulator::Now() ) return;

		    TrackTime = Simulator::Now();
	 ...
	 std::cout << Simulator::Now() << "\t"\
	    	   ...
	    	   for (int i = 0; i<= 4 ; i++)
	    	   {
	    		   std::cout <<"\t" <<  m_servHits[i] << "\t" ;
	    	   }
	    	   std::cout << std::endl;

	 /**
	    	   std::cout << Simulator::Now() << "\t"\
								 <<"ServerHitsForInstance" << "\t" ;
	   std::cout << "\t" << "NodeID: " << GetNode ()->GetId () <<"\t" ;
				   for (int i = 0; i<= 4 ; i++)
				   {

					   std::cout <<"\t" <<  m_servHitsForInstance[i] << "\t" ;
				   }
				   std::cout << std::endl;
				   **/
}
...
Producer::OnInterest (Ptr<const Interest> interest)
{
  ...
  if (interest->GetPayload ()->PeekPacketTag (dataSaveTag))
     {
	   m_servHits[ dataSaveTag.Get()-1 ]++ ;
	   m_servHitsForInstance[ dataSaveTag.Get()-1 ]++;
       data->GetPayload ()->AddPacketTag (dataSaveTag);
       NS_LOG_DEBUG(  "hooray " << "    get the packet no. is   "<<dataSaveTag.Get() );
     }


  	  NS_LOG_DEBUG(  "hooray " << "    get the packet hopcout is   "<<hopCountTag.Get() );
  m_face->ReceiveData (data);
  m_transmittedDatas (data, this, m_face);
}

std::vector<int> Producer::m_servHits(5) ; //类静态变量初始化
