  ...
  if (m_node == 0)
    {
      ...
      if (m_node != 0)
        {
    	 /*
    	  std::cout << m_forwardingStrategy->aTestSomething() ;
    	  std::cout << std::endl ;
    	  std::cout << "aljaljdglajjdslfjajdslfjadjf";
    	  std::cout << std::endl;
    	  */
  ...
  if (m_forwardingStrategy == 0)
    {
      ...
      //std::cout << "调用getobject获得fw的实例" << std::endl;
    }
...
L3Protocol::AddFace (const Ptr<Face> &face)
{
  ...
  //RegisterProtocolHandlers  ---->    brief Register callback to call when new packet arrives on the face
