...
ConsumerZipfMandelbrot::SetNumberOfContents (uint32_t numOfContents)
{
  ...
  for (uint32_t i=1; i<=m_N; i++)
    {
	  ///卧槽...他竟然这么改的...这个for里的m_pcum与上一个for里的完全不同啊.
      m_Pcum[i] = m_Pcum[i] / m_Pcum[m_N]; ///的确是累计总数,但是还是 0.1 , 0.15,  0.18 ,  0.19 ,这样第一个的频率最大.
...
ConsumerZipfMandelbrot::SendPacket() {
  ...
  if (seq == std::numeric_limits<uint32_t>::max ()) //no retransmission
    {
      ...
      seq = ConsumerZipfMandelbrot::GetNextSeq();// 就是
      m_seq ++;
    }
  ...
  nameWithSequence->appendSeqNum (seq);// 再发这个seq no的包
  //
